  ...
  return (
    <div>
      <h2 id="movie-heading" data-cy="MovieHeading">
        <Translate contentKey="moviesAppApp.movie.home.title">Movies</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="moviesAppApp.movie.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/movie/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="moviesAppApp.movie.home.createLabel">Create new Movie</Translate>
          </Link>
        </div>
      </h2>
        ...
        {movieList && movieList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="moviesAppApp.movie.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="moviesAppApp.movie.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('duration')}>
                  <Translate contentKey="moviesAppApp.movie.duration">Duration</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('duration')} />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="moviesAppApp.movie.description">Description</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('description')} />
                </th>
                <th className="hand" onClick={sort('language')}>
                  <Translate contentKey="moviesAppApp.movie.language">Language</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('language')} />
                </th>
                <th className="hand" onClick={sort('imageUrl')}>
                  <Translate contentKey="moviesAppApp.movie.imageUrl">Image Url</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('imageUrl')} />
                </th>
                <th className="hand" onClick={sort('publishDate')}>
                  <Translate contentKey="moviesAppApp.movie.publishDate">Publish Date</Translate>{' '}
            ...
            <tbody>
              {movieList.map((movie, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`moviesAppApp.Langue.${movie.language}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="moviesAppApp.movie.home.notFound">No Movies found</Translate>
            </div>
          )
        )}
      </div>
