  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="moviesAppApp.movie.home.createOrEditLabel" data-cy="MovieCreateUpdateHeading">
            <Translate contentKey="moviesAppApp.movie.home.createOrEditLabel">Create or edit a Movie</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField label={translate('moviesAppApp.movie.name')} id="movie-name" name="name" data-cy="name" type="text" />
              <ValidatedField
                label={translate('moviesAppApp.movie.duration')}
                id="movie-duration"
                name="duration"
                data-cy="duration"
                type="text"
              />
              <ValidatedField
                label={translate('moviesAppApp.movie.description')}
              ...
              />
              <ValidatedField
                label={translate('moviesAppApp.movie.language')}
                id="movie-language"
                name="language"
                data-cy="language"
                type="select"
              >
                {langueValues.map(langue => (
                  <option value={langue} key={langue}>
                    {translate('moviesAppApp.Langue.' + langue)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('moviesAppApp.movie.imageUrl')}
                id="movie-imageUrl"
                name="imageUrl"
                data-cy="imageUrl"
                type="text"
              />
              <ValidatedField
                label={translate('moviesAppApp.movie.publishDate')}
              ...
              />
              <ValidatedField
                label={translate('moviesAppApp.movie.membreStaff')}
              ...
              <ValidatedField
                label={translate('moviesAppApp.movie.category')}
                id="movie-category"
                data-cy="category"
                type="select"
                multiple
